...
export const Route = createFileRoute('/medical-logs/my-animals')({
  component: MyAnimalsMedicalLogsPage,
});

function MyAnimalsMedicalLogsPage() {
  ...
  // Filter to only logs belonging to the current foster user
  const myLogs = useMemo(() => {
    return medicalLogs.filter((log) => log.foster_user_id != null);
  }, [medicalLogs]);
  ...
  const filtered = useMemo(() => {
    return myLogs
      .filter((log) => {
        ...
        const matchesType = logTypeFilter === 'all' || log.category === logTypeFilter;
  ...
  }, [myLogs, search, logTypeFilter, sortBy]);

  // Stats scoped to user's animals
  const totalLogs = myLogs.length;
  const medicalCount = myLogs.filter((l) => l.category === 'MEDICAL').length;
  const behavioralCount = myLogs.filter((l) => l.category === 'BEHAVIORAL').length;
  const veterinaryCount = myLogs.filter((l) => l.category === 'VETERINARY').length;

  // Reduced column set for foster users
  const columns = [
        ...
        <div className="space-y-6">
          ...
          {!medicalLogsLoading && filtered.length === 0 ? (
            <p className="text-muted-foreground text-center py-12">
              No medical logs found for your animals.
            </p>
          ) : (
            <ReusableTable
              columns={columns}
